...
// Rough microbenchmark for the promoted `Batcher` (serial + deduplicated), NOT a correctness gate.
// Everything is PRINTED via `ZIO.debug`, not asserted on timings — wall-clock is noisy, especially on CI.
// Opt-in only: the whole suite is gated behind `-Dbenchmarks=true` (`ifPropSet`), so a plain `common/test`
...
object BatcherBenchSpec extends ZIOSpecDefault:
  ...
  def spec = suite("Batcher rough microbenchmark")(
    test("prints raw vs batcher per-call cost for ~0ms and 1ms logic; batcher stays correct") {
      for
        out <- ZIO.scoped(Batcher.serial(1, fastLogic).flatMap(_.run(7)))
    ...
    test("N concurrent calls: the batcher coalesces to far fewer downstream calls") {
      ...
      ZIO.scoped:
        for
          ...
          batcher  <- Batcher.serial(1024, serialisedLogic(batGate, batCalls))
    ...
    test("N concurrent over few keys: dedup collapses duplicate work") {
      ...
      ZIO.scoped:
        for
          serItems <- Ref.make(0)
          serial   <- Batcher.serial(1024, countingLogic(serItems, keys))
          serRes   <- ZIO.foreachPar(input)(serial.run)
          serI     <- serItems.get
          dedItems <- Ref.make(0)
          dedup    <- Batcher.deduplicated(1024, (i: Int) => i % keys, countingLogic(dedItems, keys))
    ...
    test("N concurrent over few keys with per-item downstream cost: dedup wins on wall-clock") {
      ...
      ZIO.scoped:
        for
          serItems <- Ref.make(0)
          serial   <- Batcher.serial(1024, perItemLogic(serItems, keys, perItem))
          serStart <- Clock.nanoTime
          serRes   <- ZIO.foreachPar(input)(serial.run)
          serEnd   <- Clock.nanoTime
          serI     <- serItems.get
          dedItems <- Ref.make(0)
          dedup    <- Batcher.deduplicated(1024, (i: Int) => i % keys, perItemLogic(dedItems, keys, perItem))
    ...
    test("one hot key, N duplicates: dedup holds O(1) state + allocations where Serial holds O(N)") {
      ...
      ZIO.scoped:
        for
          serMax   <- Ref.make(0)
          serItems <- Ref.make(0)
          serial   <- Batcher.serial(Int.MaxValue, hotKeyLogic(serMax, serItems))
          serRes   <- ZIO.foreachPar(input)(_ => serial.run(1))
          serM     <- serMax.get
          serI     <- serItems.get
          dedMax   <- Ref.make(0)
          dedItems <- Ref.make(0)
          dedup    <- Batcher.deduplicated(Int.MaxValue, (_: Int) => 0, hotKeyLogic(dedMax, dedItems))
