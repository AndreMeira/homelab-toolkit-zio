...
trait Tool[Ctx, Input, Output] {
   ...
   * outliving the call names it here. [[Tool.Result.answered]] is the rest of them.
   *
...
object Tool {
  ...
  /**
   * What a tool produced: a value, and what that value leaves outstanding.
   *
   * The same type carries a tool's own output and the rendering of it that reaches the model, which is why
   * it is parameterised — registration turns a `Result[Out]` into a `Result[String]` and changes nothing
   * else about it.
   *
   * @param value what the tool produced
   * @param standing what it opened or closes
   * @tparam A what the value is: the tool's own type before registration, text after it
   */
  final case class Result[A](value: A, standing: Result.Standing)
  ...
  object Result {
     ...
     */
    def answered[A](value: A): Result[A] = Result(value, Standing.Answered)
  }
   ...
   * @param result the tool's result as text, and what it leaves outstanding
   */
  final case class Outcome(callId: String, result: Result[String])
   ...
   * @param logic decode, run, render — with every model-actionable failure already rendered as text
  ...
  final case class Registered[Ctx](
    ...
    logic: (Ctx, String) => UIO[Result[String]]
  ) {

    /**
     * Run this tool for one caller against the arguments a model wrote.
     *
     * @param context the caller context
     * @param arguments the JSON the model produced for this call
     * @return the result, with anything the model could react to rendered as its text; never fails
     */
    def invoke(context: Ctx, arguments: String): UIO[Result[String]] = logic(context, arguments)
    ...
    private def register[In: Schema, Out: Schema](
      ...
      Registered(tool.name, tool.description, described, tool.permits)((context, arguments) =>
        ZIO
          ...
          .fold(
            reason => Result.answered(s"error: $reason"),
            result =>
              Result(JsonCodec.jsonEncoder(summon[Schema[Out]]).encodeJson(result.value).toString, result.standing),
          )
      )
  ...
  final class Session[Ctx](permitted: ListMap[String, Registered[Ctx]], context: Ctx) {
    ...
    def dispatch(call: Call): UIO[Outcome] =
      permitted.get(call.name) match
        case None       => ZIO.succeed(Outcome(call.id, Result.answered(s"error: no tool '${call.name}' is available")))
