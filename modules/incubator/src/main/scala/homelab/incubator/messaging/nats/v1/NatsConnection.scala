 ...
 * inmemory lifecycle — but it is '''fire-and-forget''': no acknowledgement, no durability, no
 ...
 *     many consumers share the inmemory's threads (O(1), not O(consumers)).
...
object NatsConnection:
  ...
  /**
   * Open a inmemory to `url`, closing it when the scope closes.
   *
   * @param url the NATS server URL (e.g. `nats://localhost:4222`)
   * @return the live inmemory; aborts with [[NatsError.Connect]] if connecting fails
   */
