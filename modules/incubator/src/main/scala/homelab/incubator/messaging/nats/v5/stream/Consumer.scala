...
class Consumer[A: Serde](
  ...
  override def consume[E2 >: NatsError](logic: A => IO[E2, Unit]): IO[E2, Unit] =
    for
      ...
      _       <- decoded match
                   case None        => ZIO.unit // already termed in decode (Discard) — nothing more to settle
                   case Some(value) =>
                     StreamPoll.heartbeat(heartbeat, message :: Nil)(logic(value).either).flatMap(handleResult(message, _))
    yield ()
