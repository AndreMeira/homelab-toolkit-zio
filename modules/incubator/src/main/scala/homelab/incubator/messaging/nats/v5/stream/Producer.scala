...
/**
 * A durable JetStream producer — `emit` waits for the server's `PublishAck`, so the message is persisted
 * (or the emit fails). The subject is the partition key, derived from the message by `subjectOf`; a
 * JetStream stream must already capture that subject for the publish to be accepted.
 *
 ...
 */
class Producer[A: Serde](jetStream: JetStream, subjectOf: A => String) extends ProducerContract[NatsError, A]:
