...
object Batcher:
  ...
  class Single[R, E, BE, In, Out](
    batchSize: Int,
    capturedScope: Scope,
    ref: Ref[Batcher.State[LineageMismatch | E | BE, In, Out]],
    ...
    // now `InFlight`, no later caller forks one either, wedging the batcher. Only the wait is interruptible.
