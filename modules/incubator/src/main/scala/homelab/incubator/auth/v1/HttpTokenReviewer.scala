...
object HttpTokenReviewer:
  ...
  private final case class ReviewRequest(apiVersion: String = "authentication.k8s.io/v1", kind: String = "TokenReview", spec: ReviewSpec) derives JsonEncoder
  private final case class ReviewSpec(token: String, audiences: Option[List[String]]) derives JsonEncoder
  private final case class ReviewResponse(status: Option[ReviewStatus]) derives JsonDecoder
  private final case class ReviewStatus(authenticated: Option[Boolean], user: Option[ReviewUser]) derives JsonDecoder
  private final case class ReviewUser(username: Option[String]) derives JsonDecoder

  private final case class HttpError(status: Int)            extends RuntimeException(s"HTTP $status")
  private final case class MalformedResponse(reason: String) extends RuntimeException(reason)
