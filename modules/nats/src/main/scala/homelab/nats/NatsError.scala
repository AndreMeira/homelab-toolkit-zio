...
enum NatsError extends AdapterError:
  ...
  /** Receiving the next message failed mid-consumption (e.g. a dropped connection) — retryable. */
  ...
  override def message: String = this match
    case Connect(cause) => s"NATS connection or setup failed: ${cause.getMessage}"
